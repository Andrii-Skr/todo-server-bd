...
export const notesThunk =
  (): ThunkAction<void, NoteState, unknown, Action<string>> => async (dispatch) => {
    try {
...
export const editThunk =
  (note: Note): ThunkAction<void, NoteState, unknown, Action<string>> =>
  async (dispatch) => {
    try {
      dispatch({ type: noteAction.loading });
      const response = await NoteService.editNote(note);
      dispatch({ type: noteAction.update, payload: response });
    } catch (e) {
      console.log(e);
    }
  };
...
export const removeThunk =
  (id: string): ThunkAction<void, NoteState, unknown, Action<string>> =>
  async (dispatch) => {
    try {
      dispatch({ type: noteAction.loading });
